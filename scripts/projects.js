...
export function loadProjects(lang) {
  fetch("projects.json")
    .then(res => res.json())
    .then(data => {
      projects = data[lang] || [];
...
export function showProject(index, instant=false) {
  ...
  const pTitle = document.createElement("h3"); pTitle.textContent = p.title;
  ...
  actions.appendChild(link); actions.appendChild(langs);

  card.appendChild(pTitle); card.appendChild(desc); card.appendChild(actions);
